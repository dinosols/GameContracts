...
pub enum GameMetadataError {
    ...
    // You must be the Player Authority and signer on this transaction
    #[error("You must be the mint authority and signer on this transaction")]
    NotPlayerAuthority,

    // Player Authority provided does not match the authority on the player
...
impl PrintProgramError for GameMetadataError {
    fn print<E>(&self) {
        msg!(&self.to_string());
    }
}

impl From<GameMetadataError> for ProgramError {
    fn from(e: GameMetadataError) -> Self {
        ProgramError::Custom(e as u32)
    }
}

impl<T> DecodeError<T> for GameMetadataError {
